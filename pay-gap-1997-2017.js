function PayGapTimeSeries() {
  ...
  this.draw = function() {
    ...
    for (var i = 0; i < this.data.getRowCount(); i++) {
      ...
      if (previous != null) {
        // Exaggerate visual difference for small gaps
        let darkColor = color(130, 0, 0);       // dark red
        let lightColor = color(255, 230, 230);  // very light pink

        let gapMagnitude = abs(current.payGap);
        let exaggeration = 3.5;
        let exaggeratedGap = pow(gapMagnitude / this.maxPayGap, exaggeration);

        let c = lerpColor(lightColor, darkColor, exaggeratedGap);
        stroke(c);
        strokeWeight(2);
